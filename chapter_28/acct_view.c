...
#include <sys/acct.h>
#include <limits.h>
#include "ugid_functions_tlpi.h"  /* Decdlaration of userNameFromId() */
#include "tlpi_hdr.h"

#define TIME_BUF_SIZE 100

static long long comptToLL(comp_t ct)  /* Convert comp_t value into long long */
{
...
int main(int argc,char *argv[])
{
    ...
    while((numRead = read(acctFile,&ac,sizeof(struct acct))) > 0)
    {
...
#else   /* Many other implementions provide ac_stat instead */
        ...
        printf("%5.2f %7.2f ",(double)(comptToLL(ac.ac_utime) + comptToLL(ac.ac_stime)) / sysconf(_SC_CLK_TCK),
